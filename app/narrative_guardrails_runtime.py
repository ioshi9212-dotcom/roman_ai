...
def _pov_activity_rule() -> Dict[str, Any]:
    return {
        ...
        "automatic_without_player_input": [
            ...
            "поддержание разговора по характеру POV; юмор, сарказм, подкол или огрызание только когда они естественны именно этому POV в этот момент",
        ...
        "instruction": (
            ...
            "Если идёт обычный разговор и ответ не является значимым выбором, POV НЕ должен искусственно молчать: отвечай за него по установленному характеру, комментируй, задавай очевидные вопросы и допускай несколько обычных обменов репликами за ход. Юмор, сарказм и подкол не являются режимом по умолчанию и возникают только когда естественны этому POV и ситуации. "
...
def _dialogue_naturalism_rule() -> Dict[str, Any]:
    return {
        ...
        "ordinary_reply_modes": [
            "буквальный и простой ответ",
            "короткая бытовая реплика",
            "спокойный или тёплый ответ",
            "раздражённый, усталый или сухой ответ без шутки",
            "неловкая или неидеальная формулировка",
            "пауза, молчание или смена темы, если это естественно персонажу",
        ],
        "avoid": [
            "цепочка punchline→контрпанч→ещё punchline как основной ритм обычной сцены",
            "одинаковый сухой сарказм у нескольких персонажей подряд",
            "автоматическая ответная шутка на любую смешную реплику",
            "псевдоофициальные остроумные формулировки у всех персонажей независимо от их голоса",
            "превращение бытового разговора в постоянный стендап до тех пор, пока POV не поднимет серьёзную тему",
        ],
        "voice_check": (
            "Перед commit мысленно убери имена говорящих. Если несколько персонажей звучат взаимозаменяемо из-за одинакового сарказма, "
            "ритма шуток или способа строить остроты, перепиши часть реплик по их собственному характеру, отношениям, настроению и привычной речи."
        ),
        "humor_budget_semantics": (
            "Указанный в scene_builder процент сарказма относится к общему тону сцены, а не к каждой реплике и не к каждому персонажу. "
            "Смешная реплика может остаться без комедийной добивки: другой персонаж может ответить буквально, усмехнуться, проигнорировать шутку, сменить тему или промолчать."
        ),
        "instruction": (
            "Диалог должен звучать как разговор разных людей, а не как общий стендап-ритм. Не делай юмор, сарказм и остроумную добивку поведением по умолчанию. "
            "Сохраняй индивидуальный голос каждого персонажа: один может шутить чаще, другой отвечать буквально, третий быть сухим, неловким, серьёзным, раздражённым или молчаливым. "
            "Одна шутка не требует ответной шутки. Обычная бытовая реплика имеет право быть просто обычной."
        ),
    }
...
def _physical_clarity_rule() -> Dict[str, Any]:
    return {
        ...
        "selective": True,
        "applies_when": [
            "романтическое, чувственное или интимное физическое сближение",
            "поцелуи, объятия, прикосновения, раздевание или изменение положения тел",
            "лечение, уход за телом, танец, тренировка, драка и другое действие, где важны дистанция и положение",
            "любой переход, который читатель иначе не сможет восстановить без догадки",
        ],
        "concrete_action_sequence_required": True,
        "sensations_supplement_action_not_replace_it": True,
        "non_graphic_is_enough": True,
        "use_for_clarity": [
            "кто куда повернулся, сел, лёг, встал или переместился",
            "какой рукой или частью тела персонаж коснулся другого, если это важно для понимания",
            "где находится другой персонаж относительно POV: рядом, за спиной, перед ним, сверху/снизу по позиции, у края кровати и т.п.",
            "что произошло с одеждой, если она участвует в действии",
            "поцелуи, прикосновения, давление, тепло, дрожь, дыхание, голос, звуки, паузы, темп и непосредственная реакция",
            "какое конкретное действие непосредственно вызвало следующую реакцию или смену положения",
        ],
        "avoid": [
            "эвфемизм вместо самого физического действия, если из него непонятно, что произошло",
            "фразы вроде «убрал последнюю дистанцию», «ответила движением», «между ними всё изменилось» как единственное описание перехода",
            "ощущения, дыхание и эмоции вместо описания реального действия",
            "монтажный скачок между двумя положениями тел без понятного перехода",
            "избыточная анатомическая или графическая детализация, если она не нужна для ясности сцены",
        ],
        "pre_commit_check": (
            "Можно ли без перечитывания и догадки восстановить последовательность физических действий и положение участников? "
            "Если нет, конкретизируй только недостающий переход. Не раздувай сцену каталогом микродвижений."
        ),
        "instruction": (
            "Важная физическая сцена может быть неграфичной, но не туманной. Пиши достаточно прямо, чтобы было понятно, кто что сделал, "
            "как изменилось положение тел или контакт и какая непосредственная реакция последовала. Ощущения, звуки и дыхание усиливают действие, а не заменяют его. "
            "Не используй эвфемизм там, где он скрывает саму физику происходящего."
        ),
    }
