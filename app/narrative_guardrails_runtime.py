...
def _scene_quality_rule() -> Dict[str, Any]:
    return {
        "mandatory": True,
        "instruction": (
            "Сцена либо меняется, либо сжимается. Быт→характер/отношения/инфо; романтика→эмоциональная дистанция/желание; интим→ясно/чувственно/непрерывно; "
            "экшн/война→риск/тактика/цена; триллер/сверхъестественное→улика/угроза/нарушение; драма→позиция/последствие. "
            "Если любой разумный следующий вариант POV ничего существенно не меняет, это не выбор. Не стоп на перемещении, ожидании, повторной проверке или техшаге. "
            "Конец естественно тянет дальше без случайного клиффхэнгера."
        ),
    }
