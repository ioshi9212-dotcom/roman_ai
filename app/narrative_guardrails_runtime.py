...
def _pov_activity_rule() -> Dict[str, Any]:
    return {
        ...
        "automatic_without_player_input": [
            ...
            "шутка, сарказм, подкол, огрызание или поддержание разговора по характеру POV",
        ...
        "instruction": (
            ...
            "Если идёт обычный разговор и ответ не является значимым выбором, POV НЕ должен искусственно молчать: отвечай за него по установленному характеру, шути, поддевай, комментируй, задавай очевидные вопросы и допускай несколько обычных обменов репликами за ход. "
...
def _scene_quality_rule() -> Dict[str, Any]:
    return {
        "mandatory": True,
        "core": (
            "Сцена существует не ради процесса, а ради изменения. Тихая сцена допустима; пустая сцена нет. "
            "Каждый содержательный beat либо меняет что-то существенное, либо путь до следующего такого beat сжимается."
        ),
        "meaningful_change": [
            "новая или изменившаяся информация",
            "риск, угроза, цена решения или доступная возможность",
            "цель, тактика или понимание задачи",
            "отношения, эмоциональная дистанция, желание, доверие, ревность или конфликт",
            "понимание персонажа, убеждение или позиция",
            "последствие решения или изменение ситуации в мире",
            "закрытие текущего вопроса, появление нового вопроса или естественное завершение сцены",
        ],
        "not_enough_alone": [
            "простое перемещение или смена физической позиции",
            "повторный взгляд, наблюдение, ожидание или проверка того же объекта без новой информации",
            "выполнение уже понятного приказа без нового риска, последствия или решения",
            "еда, курение, душ, дорога, обычная работа и другая рутина без изменения отношений, информации, конфликта или цели",
            "течение времени само по себе",
        ],
        "choice_gate": (
            "Перед возвратом управления спроси: если POV сейчас выберет любой разумный вариант следующего действия, "
            "изменится ли что-нибудь существенное? Если нет, это не выбор и сцену надо продолжить самому."
        ),
        "state_delta_test": (
            "Сравни состояние истории до и после хода. Если не изменились информация, риск, цель, отношения/эмоция, "
            "конфликт, понимание, последствия, открытый вопрос или направление сцены, не выделяй этот процесс в отдельный ход."
        ),
        "scene_promise": (
            "В начале сцены внутренне пойми, зачем читатель находится именно здесь и что этот тип сцены должен дать. "
            "К завершению обещание должно быть выполнено, изменено или осознанно закрыто; процесс без отдачи сжимается."
        ),
        "scene_types": {
            "domestic": (
                "Быт не обязан двигать крупный сюжет, но должен раскрывать характер, отношения, юмор, неловкость, конфликт, "
                "социальную динамику, мелкую новую информацию или последствия. Если люди просто едят/едут/курят/моются/работают, сожми."
            ),
            "romance": (
                "Меняй эмоциональную дистанцию, желание, доверие, ревность, уязвимость, ожидания или понимание друг друга. "
                "Показывай конкретно и кинематографично через реплики, поведение, взгляды, дистанцию и жесты."
            ),
            "intimacy": (
                "Если интимная сцена не пропущена игроком, веди её непрерывно, чувственно и ясно: кто инициирует, как другой реагирует, "
                "как меняются контакт, темп, уверенность и эмоциональная близость. Не перескакивай важные части и не прячь действие в туман."
            ),
            "action_war": (
                "Каждый новый beat меняет тактическую ситуацию, угрозу, возможность, цель, ресурс, информацию или цену решения. "
                "Последствия могут быть тяжёлыми, если причинны и подходят канону; не создавай их случайно. Наблюдение и перемещение без новой цены сжимай."
            ),
            "thriller_supernatural": (
                "Двигай неопределённость через новые свидетельства, противоречия, угрозу, нарушение известного правила или изменение понимания. "
                "Beat отвечает на вопрос, усложняет его или создаёт новый; повтор одного сигнала без новой информации не растягивай."
            ),
            "drama_conflict": (
                "Beat меняет позицию, отношение, убеждение, цену решения, расклад между людьми или последствия сказанного/сделанного. "
                "Повтор одной эмоции без развития сжимай."
            ),
        },
        "ending_pull": (
            "Конец хода должен давать естественную тягу вперёд: значимый выбор, ожидание, вопрос, напряжение, желание, угрозу, "
            "последствие, эмоциональную незавершённость или переход к следующему содержательному моменту. Не создавай случайный клиффхэнгер."
        ),
        "scene_progressed_semantics": (
            "scene_progressed=true не оправдывается одной сменой позиции, временем, повторным наблюдением или техническим действием. "
            "Физическое движение считается прогрессом только когда оно меняет риск, доступную возможность, отношения, информацию, цель или последствия."
        ),
        "instruction": (
            "Не превращай интерактивность в меню микродействий. Если варианты отличаются только тем, куда посмотреть, сколько ещё подождать, "
            "идти ли по уже данному маршруту или выполнить ли очевидный следующий шаг, они не заслуживают возврата управления. "
            "Либо дай содержательное изменение, либо сожми путь до него."
        ),
    }
