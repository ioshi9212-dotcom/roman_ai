...
def _pov_activity_rule() -> Dict[str, Any]:
    return {
        ...
        "passive_observation_alone_is_not_activity": True,
        "reserved_for_player": (
        ...
        "rule": (
            ...
            "достаточное присутствие. Минимум одно из этих проявлений должно быть активным: обычная реплика, мелкое действие "
            "или непосредственная реакция; одни ощущения, наблюдения и описание состояния не считаются достаточной активностью. "
            "Если к POV обращаются и ответ не является значимым выбором, POV отвечает словами по характеру; не заменяй естественный "
            "ответ кивком, улыбкой, взглядом или молчанием. Допускай несколько обычных обменов репликами. Остановись только перед "
            "действительно значимым решением из reserved_for_player."
        ),
    }
