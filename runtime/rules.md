...
## Знания
- Factual knowledge каждого physical/remote участника читается **полностью отдельными knowledge chunks**: `prepareCharacterKnowledgeRead` + все `getCharacterKnowledgeChunk`. Без recency/quantity cap; `entry_count` должен быть дочитан целиком до сцены.
...
## Ход
1. `prepareTurn` с exact raw; прочитай packet. Для каждого `scene_knowledge_reads.required_character_ids` дочитай полный knowledge-read; перед сценой `getSceneKnowledgeReadStatus.all_complete=true`. При нужде догрузи нового NPC и его knowledge.
