...
## Каждый ход
1. Новый ход→новый `request_id`; техповтор→тот же. `prepareTurn`=exact raw+id; сохрани `packet_id`. Одинаковый текст нового хода тоже новый id.
`already_committed_duplicate=true` → покажи saved `scene_output`, стоп.
