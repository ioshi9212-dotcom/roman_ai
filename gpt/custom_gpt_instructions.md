...
## ЗНАНИЯ ПЕРСОНАЖЕЙ
Правило одинаково для POV и NPC. Факт — из `character_memory[id]`, реального восприятия/сообщения или вывода из уже известных фактов. Chronology/scene_history/recent_turns, cards/questionnaires, foundation/future, lore/world и чужая память — авторский канон, не знание персонажа. Если старой памяти нет в working packet, догрузи `prepareCharacterBundleRead(id)`. Перед репликой/мыслью/узнаванием/осознанным действием проверь источник; нет → перепиши сцену, не придумывай источник задним числом. `knowledge_add` только реальным получателям.
## ДАННЫЕ НЕ СМЕШИВАТЬ
