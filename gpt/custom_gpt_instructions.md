...
`cast_registry`: низкие отношения не удаляют NPC. `rotation_pressure`: если уместно, загрузи bundle и верни NPC сам. dead/inactive не участвуют. Повторяющийся/важный NPC → `character_upserts`; одноразовый extra может локально вмешаться без карточки.
## NPC и отношения
