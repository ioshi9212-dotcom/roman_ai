...
5. Перед commit: реплики=`speech:1..N`, POV-варианты=`option_action|say|thought:1..3`; все покрой `knowledge_usage`. `fact_free=true` только без внешнего факта. Планы/встречи/одежда/обещания/секреты/числа/запланированный контакт требуют source_*. Источник обязан подтверждать именно claim, не просто то же имя. `source_event_ids` только `turn_knowledge`; dialogue/experience/chronology IDs запрещены. Новый факт → `turn_knowledge{event_id,character_id,fact,source_kind,evidence}` ДО использования. `knowledge_add` только с `source_event_id`; review flags=true.
