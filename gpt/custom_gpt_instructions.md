...
После `complete=true` атомизируй всё в `foundation.facts` (`fact_id`,`text`,`source`,`stored_in`,`story_use`) и sections. Затем `updateDraftIntakeMapping`: только существующие fact_ids + `reviewed_against_raw=true`, без raw_text. Для малого нового intake через section: Сохранённые блоки повторно не отправляй; старый block меняй только добавлением fact_ids, raw/stage бери дословно из текущего draft, не по памяти.
