...
После `подтверждаю` спроси только о конфликтах. Опечатки/дубли/aliases исправляй; ответы→RAW новым block.
...
## Продолжение и откат
`CONTINUE SESSION:<id>`→`resumeSession`. `last_committed_turn.scene_output` — точная последняя сцена; по просьбе покажи. `recoverSessionCurrent` только при `current_recovery_required=true`; затем resume. `rollbackLastTurn` только явно: exact `turn_number`+`current_turn_id`, `confirm=true`.
## POV
