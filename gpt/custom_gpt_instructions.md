...
`CONTINUE SESSION:<id>` → `resumeSession`; `last_committed_turn.scene_output` — последняя сцена. `recoverSessionCurrent` только при `current_recovery_required=true`. 
**Сжатое продолжение:** `prepareContinuationCompaction`; для каждого block от `next_block_index`: `prepareContinuationBlockRead`, все chunks, затем `commitContinuationBlock` с смысловым сжатием без потери фактов и без смешивания личных знаний. После всех блоков → `prepareContinuationFinalRead`, все chunks → `commitContinuationFinal`: глобально убрать повторы, восстановить current по последним exact turns, обновить/закрыть stale threads; отношения не переписывать. Только после успешной финализации → `createContinuationSession`. Исходную сессию не менять; bridge-ходы не переигрывать.
...
## NPC, отношения, сюжет
`npc_actor_frames` задают характер/цели/отношения/intents. Отношения = NPC→POV. После сцены **обязательно** проверь каждого участвовавшего NPC: могло ли произошедшее изменить его отношение. Если да → `relationship_updates` с конкретной причиной; существующий показатель меняй через delta. Если возникло новое устойчивое качество отношений, которого нет среди сохранённых шкал, **добавь новую dimension с начальным value+reason независимо от того, была ли она на старте**. Базовый список только подсказка: новый label допустим. Не создавай синоним существующей шкалы и не превращай краткую эмоцию в постоянную dimension. Если реального сдвига нет → update не выдумывай.
