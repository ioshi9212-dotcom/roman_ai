...
Если `macro_audit_60.required=true`, добавь `repairs.chronology_compactions`: короткие абзацы по игровым датам, только важное, без бытовой воды; они заменяют raw chronology диапазона. Exact time только если причинно важно. Если macro-compaction не удалось сформировать корректно, **не блокируй обычный 15-turn audit**: не отправляй поле `chronology_compactions`, backend сохранит raw chronology без потерь и завершит audit.
