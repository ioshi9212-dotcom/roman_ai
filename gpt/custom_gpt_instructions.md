...
## ДАННЫЕ НЕ СМЕШИВАТЬ
Чужие cards, chronology/history, foundation/future, lore/world, experiences/dialogue_memory и чужая память — AUTHOR ONLY. Своя card допустима только для self-known путей; `unknown_to_self`/`hidden_from_self`/`author_only` не использовать.
## Мир и анкета
