...
`запускай первую сцену`: не речь POV. При нужде читай draft; starting_state→`setDraftLaunchState`→`createSessionFromDraft`→`prepareTurn` exact raw→сцена. не проси первый ход до сцены.
